...
WHERE i.status = $issued_id AND i.user_id = $user_id
ORDER BY 
    b.book_name ASC;";
    $stmt = $pdo->prepare($query);
    $stmt->execute();
                            ...
                            <td class="text-truncate text-capitalize">
                                <?php 
                                    ...
                                    if ($returnDate < $currentDate) {
                                        echo '<div class="bg-danger text-white p-2 rounded">Overdue</div>'; // Red block for overdue
                                    } else {
                                        echo '<div class="bg-warning text-dark p-2 rounded">To be returned</div>'; // Yellow block for to be returned
                                    }
                                ?>
                            </td>
                        </tr>
